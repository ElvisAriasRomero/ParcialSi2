...
<!-- #############################################################3######--> 
  ...
  <h1 class="text-3xl text-center font-bold">REGISTRAR DETALLE COMPRAS  </h1>
    ...
    <div class="card border-left-info shadow h-100 py-2">
        <div class="card-body">
            <div class="row no-gutters align-items-center">
                <div class="col mr-2">
                    <div class="text-xs font-weight-bold text-info text-uppercase mb-1">PROVEEDOR
                    </div>
                    <div class="row no-gutters align-items-center">
                        <div class="col-auto">
                            <div name="id_cliente" id="id_cliente" class="h5 mb-0 mr-3 font-weight-bold text-gray-800">ID: {{$user->id}}</div>
                           
                            <div class="h5 mb-0 mr-3 font-weight-bold text-gray-800">NIT: {{$user->nit}}</div>
                          
                            <div class="h5 mb-0 mr-3 font-weight-bold text-gray-800"> NOMBRE: {{$user->nombre}}</div>
                            <div class="h5 mb-0 mr-3 font-weight-bold text-gray-800">DIRECCION: {{$user->direccion}} {{$user->telefono}}</div>
...
<div class="card-header py-3">
  ...
  <form action="{{route("admin.agregarProductoCompra",$user->id)}}" method="post" class="row">
    @csrf
<div class="col-12 col-md-7">
        ...
        <div class="form-group">
            ...
            <select required class="form-control" name="codigo" id="codigo">
                @foreach($prod as $p)
                    <option value="{{$p->id}}">{{$p->nombre}}  - plazas: {{$p->plaza}} - color: {{$p->color}} - tipo: {{$p->tipo}} </option>
                @endforeach
            </select>
...
<div class="col-12 col-md-2">
    <label for="ci2">Precio</label>
    <input type="number" class="form-control" value='1200'  name="precio" placeholder="precio">
</div> 
<div class="col-12 col-md-1">
    <label for="ci3">almacen</label>
    <input type="number" class="form-control" value='1'  name="almacen" placeholder="almacen">
</div> 
</form>  
...
<!-- #############################################################3######--> 
 ...
 @if(session("productos") !== null)
                    ...
                    <body>
                        @foreach(session("productos") as $producto)
                            <tr>
                                ...
                                <td>
                                    <form action="{{route("admin.quitarProductoDeCompra",$user->id)}}" method="post">
                                        @method("delete")
                                        @csrf
           ...
           <form action="{{route("admin.terminarOCancelarCompra",$user->id)}}" method="post">
                        @csrf
                        @if(session("productos") !== null)
                            <div class="form-group">
...
<!-- AQUI TERMINA LAS OPCIONES ANTEIROESSSSSSSSSSSS########################################################################################## -->
        ...
        <script src="{{asset('bsadmin/functions_venta.js')}}"></script>
